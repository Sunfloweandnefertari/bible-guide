...
│   └── 07-讲道风格.md         # 某位老师的讲道教学法（教学节奏/比喻库/释经方式）
├── tools/                    # 音频转写与风格提取
│   ├── transcribe.py         # 批量转写（硅基流动 SenseVoice，长音频自动处理）
│   ├── clean_transcripts.py  # 转写稿清洗 + 风格指标统计
│   └── STYLE-EXTRACTION-SPEC.md  # 风格提取规范（六维度）
└── data/                     # 经文数据与工具
...
**v3 特性**
- 🎙 **讲道风格**：可从「智慧视角」一键切换到「讲道风格」——用某位老师的教学方式回应（先铺垫再讲经、比喻优先、提问引导、标注推理边界），只学方式不冒充本人
...
- 📱 **手机友好**：专为手机优化的紧凑布局，可「添加到主屏幕」当 App 用（PWA）
